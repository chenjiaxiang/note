...
同样的，因为上述辅助变量是个后验概率，含义为在HMM参数已知和观测序列确定时，在$t$时刻从状态$i$转移到状态$j$的概率。计算公式如下：
...
### 基于辅助变量的HMM参数学习过程
以上辅助变量的确定有一个前提，那就是HMM的两个参数（转移概率矩阵和发射概率矩阵）已知。但是因为我们使用的是EM算法，其实上步对应的是E-step，当辅助变量确定下来，又计算HMM参数的过程对应EM算法的M-step。这步是相当简单的，只需要把离散计算过程推广到概率计算过程就可以了，我们成为从hard计算到soft计算的转换。